...
const Addapi = () => {
    ...
    const sendrequest = async () => {
        const res = await axios.post('/api/addblogapi', blog, { headers: { "Content-Type": "application/json" } });
    ...
    return (
        <div style={{fontFamily:'Roboto, sans-serif'}}>
            ...
            <Navbar />
            {
                sucess && <div className='bg-green-200 h-60 max-w-6xl mx-4 md:mx-auto my-6 rounded-xl text-center flex items-center justify-center text-4xl text-green-700 font-extrabold'>Successfully added the Blog</div>
            }
            {
                fail && <div className='bg-red-200 h-60 max-w-6xl mx-4 md:mx-auto my-6 rounded-xl text-center flex items-center justify-center text-4xl text-red-700 font-extrabold'>Couldn&lsquo;t add the blog, try again</div>
            }
            {submit ?
            ...
            }
            {
                !sucess && !fail && <form className='max-w-7xl  mx-auto px-6 md:px-4 mb6' onSubmit={handleSubmit}>
                    <h1 className='text-3xl font-extrabold text-gray-700 my-6'>Add Blog</h1>
