        ...
        <Link href={`/blog/${data._id}`} className='py-5 flex md:flex-row space-y-3 items-center  md:space-x-6 flex-col group bg-white w-full md:h-64 md:max-h-72 cursor-pointer '>
            ...
            <div className='md:w-3/5 h-fit'>
                ...
                <div className='flex flex-wrap space-x-2 '> 
                    {data && data.tags.split(", ").slice(0,3).map((e, i)=><div key={i} className="text-xs my-1 md:my-0 py-1 px-2 flex text-gray-700 rounded-full bg-yellow-200  text-center group-hover:bg-yellow-100 group-hover:text-gray-800"># {e}</div>)}
                </div>
            </div>
        </Link>
    )
